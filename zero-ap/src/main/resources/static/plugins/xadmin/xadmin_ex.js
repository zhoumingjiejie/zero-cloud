;!function (win) {
    ...
    //url格式添加前缀
    var urlFormat = "view?name=[url]&path=xadmin";

    Xadmin.prototype.format_url = function (url) {
        //页面解析到当前为止所有的script标签
        var js = document.scripts;
        //js[js.length - 1] 就是当前的js文件的路径
        js = js[js.length - 1].src.substring(0, js[js.length - 1].src.lastIndexOf("/") + 1);
        //输出当前js文件所在的目录
        if (typeof (urlFormat) !== "undefined" && urlFormat !== null && urlFormat !== "" && url.indexOf("http") !== 0 && url.indexOf("https") !== 0) {
            url = urlFormat.replace("[url]", url);
        }
        return url;
    };
     ...
     * @param src src地址
     */
    Xadmin.prototype.replaceIframe = function (src) {
    ...
    Xadmin.prototype.add_lay_tab = function (title, url, id) {
        element.tabAdd('xbs_tab', {
            title: title
            ,
            content: '<iframe tab-id="' + id + '" frameborder="0" src="' + this.format_url(url) + '" scrolling="yes" class="x-iframe"></iframe>'
            ,
            id: id
        })
    }
