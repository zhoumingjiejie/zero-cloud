package com.github.icezerocat.zero.dynamic.authorization.config.support.token;

import com.github.icezerocat.zero.dynamic.authorization.util.Assert;
...
public class CustomAuthorizationServerTokenServices extends DefaultTokenServices {
    ...
    public CustomAuthorizationServerTokenServices(AuthorizationServerEndpointsConfigurer endpoints) {
        ...
        // 为了 TokenGenerator 能使用到父类的方法. (否则父类的 clientDetailsService 为 null)
    ...
    // ~ Methods implementing from AuthorizationServerTokenServices
    // -----------------------------------------------------------------------------------------------------------------
    ...
    //   由于现阶段的资源服务器已经实现了独立验证签名, 故实现于接口 ResourceServerTokenServices 的方法不启用
    // -----------------------------------------------------------------------------------------------------------------

    @Override
    public OAuth2AccessToken readAccessToken(String accessToken) {
        throw new UnsupportedOperationException("Authorization Server :: Unsupported.");
    }
    ...
    private final class TokenGenerator {
        ...
        public OAuth2RefreshToken createRefreshToken(OAuth2Authentication authentication) {
            ...
            final int validitySeconds = CustomAuthorizationServerTokenServices.super.getRefreshTokenValiditySeconds(authentication.getOAuth2Request());
        ...
        public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
            ...
            final int validitySeconds = CustomAuthorizationServerTokenServices.super.getAccessTokenValiditySeconds(authentication.getOAuth2Request());
            if (validitySeconds > 0) {
