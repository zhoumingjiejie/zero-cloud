 ...
 */
@Component
public class CustomVerifyListenerA extends AbsCustomVerifyListener {

    /**单例Bean,这个静态变量会缓存证书中最大用户注册数的*/
    private static Long registerAmount = 1000L;
    ...
    public boolean verify(LicenseExtraParam licenseExtra) throws ApiException {
        ...
        if(count>=registerAmount){
            throw new ApiException("系统当前用户数超过最大用户注册限制数【"+registerAmount+"】");
        }
     ...
     * @throws ApiException
     */
    public boolean verify() throws ApiException{
        Long count = 100L;
